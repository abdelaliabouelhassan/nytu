                ...
                <div class="accordion-item py-5 md:py-10 border-b-2 border-[#D9D9D9]" v-for="(item,index) in updates" :key="index">
                    <div class="accordion-header  py-2 bg-white text-primary  rounded-md flex items-center justify-between">
                      <div class=" text-3xl md:text-[5rem] font-normal"> {{ item.version }} </div>
                        <div class="text-xl  hidden md:block text-black font-normal max-w-[29.563rem]"> 
                            {{ item.title }}
                        </div>
                      
                        <svg class="chevron -rotate-180 " width="38" height="20" viewBox="0 0 38 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M1.5 18.75L19 1.25L36.5 18.75" stroke="#9099FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>

                    </div>
                    <div class="accordion-content">
                        <div class="text-base pt-4   md:hidden text-black font-normal max-w-[29.563rem]"> 
                            {{ item.title }}
                        </div>
                        <div class=" w-full flex items-center justify-between">
                        <div class=" text-[5rem] font-normal invisible hidden md:block"> V5.6 </div>
                        <div class=" text-base md:text-xl text-black font-bold w-full max-w-[29.563rem]"> 
                            <div class=" w-full flex  space-x-2 space-y-6" v-for="(content,index) in item.content" :key="index">
                                <span class="text-5xl ">.</span>
                                <span>{{ content }}</span>
                            </div>
                        </div>
                      
                        <svg class="chevron -rotate-180 invisible hidden md:block " width="38" height="20" viewBox="0 0 38 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M1.5 18.75L19 1.25L36.5 18.75" stroke="#9099FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
